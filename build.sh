...
docker build \
  ...
  --tag mauwii/devopsbuildagent \
  --tag mauwii/devopsbuildagent:linux-x64 .

export DOCKER_DEFAULT_PLATFORM="linux/arm64"
docker build \
  --build-arg='TARGETARCH=linux-arm64' \
  --tag mauwii/devopsbuildagent:linux-arm64v8 .

# STEP 2 push images to registry
docker push mauwii/devopsbuildagent
docker push mauwii/devopsbuildagent:linux-x64
docker push mauwii/devopsbuildagent:linux-arm64v8

# STEP 3 - create and push manifest
docker manifest create mauwii/devopsbuildagent:latest \
